	...
	function registerTool(): void {
		...
		pi.registerTool<typeof Parameters, SubagentDetails>({
			...
			description: [
				...
				`只读 Agent（工具白名单全部是 read/grep/find/ls，或已知只读的扩展工具 ffgrep/fffind/fff-multi-grep/multi_grep/web_search/source_check/fetch_content/get_search_content/resolve-library-id/query-docs）最多同时运行 ${MAX_CONCURRENCY} 个（父会话内所有 subagent 调用共用一个队列），多出的排队；含写入或名单外扩展工具的 Agent 无法静态判断是否写盘，会独占执行，期间不与其他子任务并行。`,
				"等待和执行均可取消，执行超时为 10 分钟；单个任务失败不影响其他任务，全部失败才报错。",
				"默认工具为只读 read, grep, find, ls；声明可信只读来源（pi-fff、pi-web-access、context7）后，省略 tools 会按来源实际注册的工具一并启用其只读工具。写入/命令工具必须在 Agent 定义里显式授予；pi 没有权限弹窗，授权只发生在配置这一步，授予后子会话会直接执行。",
